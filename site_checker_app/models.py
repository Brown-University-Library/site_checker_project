...
class CheckSite(models.Model):
    ...
    recent_checked_result = models.CharField( max_length=50, help_text='(Set automatically when script runs)', null=True, blank=True )
    previous_checked_result = models.CharField( max_length=50, editable=False, help_text='(Set automatically when script runs)', null=True, blank=True )
    pre_previous_checked_result = models.CharField( max_length=50, editable=False, help_text='(Set automatically when script runs)', null=True, blank=True )
    next_check_time = models.DateTimeField( help_text='(Set automatically when script runs)', null=True, blank=True )  # stores calculated recent_checked_time + calculated_seconds
